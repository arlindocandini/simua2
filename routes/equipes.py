...
def lista():
    ...
    equipes = Equipe.query.order_by(Equipe.nome.asc()).all()
...
def detalhes(id):
    ...
    membros = User.query.filter_by(equipe_id=id).order_by(User.nome_completo).all()

    total_os = OrdemServico.query.filter_by(equipe_id=id).count()
    os_concluidas = OrdemServico.query.filter_by(equipe_id=id, status='aprovada').count()
    os_ativas = OrdemServico.query.filter(
        OrdemServico.equipe_id == id,
        OrdemServico.status.in_(['criada', 'atribuida', 'em_andamento'])
    ).count()

    taxa_conclusao = round((os_concluidas / total_os) * 100, 1) if total_os > 0 else 0
    ultimas_os = OrdemServico.query.filter_by(equipe_id=id)\
        .order_by(desc(OrdemServico.created_at)).limit(10).all()

    usuarios_disponiveis = User.query.filter(
        User.ativo.is_(True),
        User.equipe_id.is_(None)
    ...
    if request.method == 'POST':
        try:
            ...
            equipe = Equipe(
                ...
                telefone_emergencia=telefone_emergencia.strip() if telefone_emergencia else None,
...
@admin_required
def excluir(id):
    """Exclui completamente uma equipe"""
    equipe = Equipe.query.get_or_404(id)
    os_ativas = OrdemServico.query.filter(
        OrdemServico.equipe_id == id,
        OrdemServico.status.in_(['criada', 'atribuida', 'em_andamento'])
    ...
    try:
        for membro in User.query.filter_by(equipe_id=id).all():
